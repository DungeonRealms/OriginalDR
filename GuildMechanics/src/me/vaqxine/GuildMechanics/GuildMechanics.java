package me.vaqxine.GuildMechanics;
...
public class GuildMechanics extends JavaPlugin implements Listener {
	...
	public ItemStack guild_emblem = ItemMechanics.signCustomItem(Material.EYE_OF_ENDER, (short)1, ChatColor.DARK_AQUA.toString() + ChatColor.BOLD.toString() + "Guild Emblem", ChatColor.GRAY.toString() + "Guild:" + ChatColor.GRAY.toString() + " Legendary Heroes" + ","
			+ ChatColor.DARK_AQUA + "Right Click: " + ChatColor.GRAY.toString() + "Open Guild Portal" + ","
			+ ChatColor.DARK_AQUA + "Left Click: " + ChatColor.GRAY.toString() + "Invite to Guild" + ",");
			...
			if(msg.equalsIgnoreCase("y")){
				...
				for(String s : getGuildMembers(g_name)){
					...
					guildie.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + guild_handle_map.get(g_name) 
							+ ChatColor.DARK_AQUA + "> " + ChatColor.DARK_AQUA + pl.getName() + ChatColor.GRAY + " has " + ChatColor.UNDERLINE + "left" + ChatColor.GRAY + " the guild.");
				}
			...
			if(step == 4){ // Colors given, summary given, check if they confirm creation & process.
				if(e.getMessage().equalsIgnoreCase(getUpgradeAuthenticationCode(pl.getName()))){
					...
					if(e.getMessage().equalsIgnoreCase("cancel")){
						...
						pl.sendMessage(ChatColor.GRAY + "Please enter either the confirmation code to create your guild for 5,000G, '" + ChatColor.DARK_AQUA + getUpgradeAuthenticationCode(pl.getName()) + ChatColor.GRAY + "'" + ChatColor.GRAY + " " + ChatColor.UNDERLINE + "OR" 
								+ ChatColor.GRAY + " Type '" + ChatColor.DARK_AQUA + "cancel" + ChatColor.GRAY + " to reset this entire process.");
						return;
					}
				}
			}
	...
	public void onInventoryClick(InventoryClickEvent e){
		...
		if(!(e.getInventory().getName().equalsIgnoreCase("container.crafting"))){
			return; // Not their own inventory.
		}
		if(e.isShiftClick()){
			return;
		}
	...
	public static void clearGuildTabList(Player pl, boolean update){
		...
		while(x < TabAPI.getVertSize()){
			TabAPI.setTabString(instance, pl, x, y, "" + TabAPI.nextNull());
			x++;
		}
		y = 1; x = 0;
		while(x < TabAPI.getVertSize()){
	...
	public static void updateGuildTabList(final Player pl){
		if(pl == null || !(inGuild(pl.getName()))){
			...
			TabAPI.setTabString(instance, pl, 0, 0, ChatColor.GRAY + "*------------"); 
			TabAPI.setTabString(instance, pl, 0, 1, ChatColor.DARK_AQUA + "" + ChatColor.BOLD + "    Guild UI");
			TabAPI.setTabString(instance, pl, 0, 2, ChatColor.GRAY + "  ----------*"); 

			TabAPI.setTabString(instance, pl, 2, 1, ChatColor.DARK_AQUA + "" + "Guild Name");
			TabAPI.setTabString(instance, pl, 3, 1, ChatColor.GRAY + "N/A");

			TabAPI.setTabString(instance, pl, 17, 1, ChatColor.DARK_AQUA + "Shard " + ChatColor.GRAY.toString() + getLocalServerName(), 0);
			...
			TabAPI.setTabString(instance, pl, 18, 1, ChatColor.GRAY.toString() + online_players + " / " + max_players, 0);
			TabAPI.updatePlayer(pl);
		...
		TabAPI.setPriority(instance, pl, 1);

		TabAPI.setTabString(instance, pl, 0, 0, TabAPI.nextNull() + ChatColor.GRAY + "*-------------"); 
		TabAPI.setTabString(instance, pl, 0, 1, ChatColor.DARK_AQUA + "" + ChatColor.BOLD + "    Guild UI");
		TabAPI.setTabString(instance, pl, 0, 2, ChatColor.GRAY + "  ---------*"); 

		TabAPI.setTabString(instance, pl, 2, 1, ChatColor.DARK_AQUA + "" + "Guild Name");
		TabAPI.setTabString(instance, pl, 3, 1, ChatColor.GRAY + g_name);

		TabAPI.setTabString(instance, pl, 5, 1, ChatColor.DARK_AQUA + "" + "Guild Owner", 0);
		...
		if(guild_owner.contains(ChatColor.GREEN.toString()) || guild_owner.contains(ChatColor.YELLOW.toString())){
			...
			TabAPI.setTabString(instance, pl, 6, 1, guild_owner, 0);
		}
		else{
			TabAPI.setTabString(instance, pl, 6, 1, guild_owner, 9999999);
		}
		...
		TabAPI.setTabString(instance, pl, 2, 0, ChatColor.DARK_AQUA + "" + "Officers [" + getOnlineOfficerCount(g_name) + "/" + getTotalOfficerCount(g_name) + "]", 0);

		TabAPI.setTabString(instance, pl, 2, 2, ChatColor.DARK_AQUA + "" + "Members [" + getOnlineGuildCount(g_name) + "/" + getGuildMemberCount(g_name) + "]", 0);
		...
		for(String s : pruned_members){
			...
			if(s.contains(ChatColor.GREEN.toString()) || s.contains(ChatColor.YELLOW.toString())){
				...
				TabAPI.setTabString(instance, pl, x, 2, s, 50);
			}
			else{
				TabAPI.setTabString(instance, pl, x, 2, s, 9999999);
			}
		}

		if(pruned_members.size() == 0){
			TabAPI.setTabString(instance, pl, 3, 2, ChatColor.GRAY.toString() + "N/A" + TabAPI.nextNull());
		}
		...
		for(String s : pruned_officers){
			...
			if(s.contains(ChatColor.GREEN.toString()) || s.contains(ChatColor.YELLOW.toString())){
				...
				TabAPI.setTabString(instance, pl, x, 0, s, 50);
			}
			else{
				TabAPI.setTabString(instance, pl, x, 0, s, 9999999);
			}
		}

		if(pruned_officers.size() == 0){
			// Yuck. No officers.
			TabAPI.setTabString(instance, pl, 3, 0, ChatColor.GRAY + "N/A" + TabAPI.nextNull());
		}
	...
	public static void setupGuildData(String p_name, String guild_name){
		...
		if(guild_name.equalsIgnoreCase("null")){
			return;
		}
	...
	public static String getPlayerGuildSQL(String p_name){
		...
		try {
			pst = ConnectionPool.getConnection().prepareStatement( 
					"SELECT guild_name FROM player_database WHERE p_name = '" + p_name + "'");
	...
	public static void processGuildTagExistRequest(final String p_name, final String g_tag){
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				try {
					pst = ConnectionPool.getConnection().prepareStatement( 
							"SELECT guild_handle FROM guilds WHERE guild_handle = '" + g_tag + "'");
	...
	public static void updateGuildSQL(final String guild_name){
		...
		Hive.sql_query.add("INSERT INTO guilds (guild_name, guild_handle, guild_color, members, guild_server_num) VALUES('" + guild_name + "', '" + guild_handle + "', '" + guild_color + "', '" + member_string + "', '" + home_server + "')"
	...
	public static boolean downloadGuildDataSQL(String g_name, boolean temp){
		...
		try {
			pst = ConnectionPool.getConnection().prepareStatement( 
					"SELECT guild_handle, guild_color, guild_server_num, members, motd, biography FROM guilds WHERE guild_name = '" + g_name + "'");
	...
	public static void setGuildRank(String p_name, int rank_num){
		if(!(inGuild(p_name)) || !guild_map.containsKey((getGuild(p_name)))){
	...
	public static void leaveGuild(String p_name, String g_name, boolean update_sql){
		...
		if(key_s == ""){
			return; // Never found them in guild data.
		}
	...
	public boolean isGuildDye(ItemStack is){
		if(is.getType() == Material.EXP_BOTTLE && is.hasItemMeta() && is.getItemMeta().hasDisplayName() && ChatColor.stripColor(is.getItemMeta().getDisplayName()).equalsIgnoreCase("Guild Armor Dye")){
			return true;
		}
		return false;
	}
	...
	public boolean doesGuildHandlerExistLocal(String guild_name){
		if(guild_handle_map.containsKey(guild_name)){
			return true;
		}
	...
	public static boolean inGuild(String p_name){
		if(player_guilds.containsKey(p_name)){
			return true;
		}
		return false;
	}
	...
	public static boolean areGuildies(String p1, String p2){
		...
		if(p1_guild.equalsIgnoreCase(p2_guild)){
			return true;
		}
		return false;
	}
	...
	public boolean isGuildLeader(String p_name){
	...
	public boolean isGuildOfficer(String p_name){
	...
	public void sendMessageToGuild(Player sender, String raw_msg){
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args){
		...
		if(cmd.getName().equalsIgnoreCase("gquit")){
			if(!(inGuild(p.getName()))){
			...
			String g_name = getGuild(p.getName());
			p.sendMessage(ChatColor.GRAY + "Are you sure you want to QUIT the guild '" + ChatColor.DARK_AQUA + g_name + ChatColor.GRAY + "' - This cannot be undone. " 
					+ "(" + ChatColor.GREEN.toString() + ChatColor.BOLD + "Y" + ChatColor.GRAY + " / " + ChatColor.RED.toString() + ChatColor.BOLD + "N" + ChatColor.GRAY + ")");
			if(isGuildLeader(p.getName())){
			...
			guild_quit_confirm.add(p.getName());
		}

		if(cmd.getName().equalsIgnoreCase("gkick")){
			if(!(inGuild(p.getName()))){
			...
			int g_rank = getRankNum(p.getName());
			String g_name = getGuild(p.getName());

			if(g_rank < 2){ // 2 = officer, 3 = owner -> 1 is just a member.
				p.sendMessage(ChatColor.RED + "You must be a guild " + ChatColor.BOLD + "OFFICER" + ChatColor.RED + " to use " + ChatColor.BOLD + "/gkick");
				return true;
			}

			String p_to_kick = args[0];
			
			if(!(inSpecificGuild(p_to_kick, g_name))){
				p.sendMessage(ChatColor.RED + p_to_kick + " is not in your guild.");
				return true;
			}
			
			if(getGuildRank(p_to_kick, g_name) == 2 && !(isGuildLeader(p.getName()))){
				p.sendMessage(ChatColor.RED + "You cannot kick a fellow guild officer -- Only the guild leader can do that.");
				return true;
			}

			if(getGuildRank(p_to_kick, g_name) == 3){
				p.sendMessage(ChatColor.RED + "You cannot kick the guild leader.");
				return true;
			}

			leaveGuild(p_to_kick, g_name, true); // Kick them from local server.
			...
			for(String s : getOnlineGuildMembers(g_name)){
				...
				if(pl != null){
					pl.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + guild_handle_map.get(g_name) + ChatColor.DARK_AQUA + "> " + ChatColor.DARK_AQUA + p_to_kick + " has been " + ChatColor.UNDERLINE + "kicked" + ChatColor.DARK_AQUA + " by " + p.getName() + ".");
				}
			}
		}

		if(cmd.getName().equalsIgnoreCase("ginvite")){
			if(!(inGuild(p.getName()))){
			...
			inviteToGuild(to_invite, p);	
		}

		if(cmd.getName().equalsIgnoreCase("gaccept")){
			if(args.length != 0){
				p.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "Invalid Syntax. " + ChatColor.RED + "/gaccept");
				return true;
			}

			if(!(guild_invite.containsKey(p.getName()))){
				p.sendMessage(ChatColor.RED + "No pending guilds invites.");
				return true;
			}

			String guild_name = guild_invite.get(p.getName());
			String inviter = guild_inviter.get(p.getName());

			if(!guild_map.containsKey(guild_name)){ // Data is gone.
				p.sendMessage(ChatColor.RED + "This guild invite is no longer available.");
				guild_invite.remove(p.getName());
				guild_inviter.remove(p.getName());
				guild_invite_time.remove(p.getName());
				guild_inviter.remove(p.getName());
				return true;
			}

			for(String s : getGuildMembers(guild_name)){
				if(Bukkit.getPlayer(s) != null){
					Player pty_mem = Bukkit.getPlayer(s);
					pty_mem.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + guild_handle_map.get(guild_name) + ChatColor.DARK_AQUA + "> " + ChatColor.DARK_AQUA.toString() + p.getName() + ChatColor.GRAY.toString() + " has " + ChatColor.UNDERLINE + "joined" + ChatColor.GRAY + " your guild. [INVITE: " + ChatColor.ITALIC + inviter + ChatColor.GRAY + "]");
				}
			}

			// They will always be local, just need to tell others.
			addPlayerToGuild(p.getName(), guild_name);

			String message_to_send = "[gadd]" + p.getName() + "," + guild_name + ":" + inviter;
			sendGuildMessageCrossServer(message_to_send);

			setPlayerGuildSQL(p.getName(), guild_name, false);
		...
		if(cmd.getName().equalsIgnoreCase("gdecline")){
			if(args.length != 0){
		...
		if(cmd.getName().equalsIgnoreCase("gpromote")){
			if(args.length != 1){
			...
			promoteToOfficer(p_name_2promote, p);
		}

		if(cmd.getName().equalsIgnoreCase("gdemote")){
			if(args.length != 1){
			...
			demoteOfficer(p_name_2demote, p);
		}

		if(cmd.getName().equalsIgnoreCase("gbanner")){
			if(!(inGuild(p.getName()))){
			...
			if(args.length == 1){
				...
				log.info("[GuildMechanics] Setting banner of " + g_name + " to: " + better_url);
				setGuildBannerSQL(g_name, better_url);
		...
		if(cmd.getName().equalsIgnoreCase("gbio")){
			if(!(inGuild(p.getName()))){
			...
			if(args.length == 0){
				...
				if(!(isGuildLeader(p.getName()))){
					...
					if((!(guild_bio.containsKey(getGuild(p.getName())))) || guild_bio.get(getGuild(p.getName())) == null){
						// NO MOTD.
					...
					if(guild_bio.containsKey(g_name) && guild_bio.get(g_name) != null){
						String bio = guild_bio.get(g_name);
						p.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + guild_handle_map.get(g_name) + ChatColor.DARK_AQUA + "> " + ChatColor.BOLD + "Guild Biography: " + ChatColor.DARK_AQUA + bio);
					}
					return true;
				}
				
				if(guild_bio_dynamic.containsKey(p.getName())){
				...
				guild_bio_dynamic.put(p.getName(), "");
				p.sendMessage("");
		...
		if(cmd.getName().equalsIgnoreCase("gmotd")){
			if(!(inGuild(p.getName()))){
			...
			if(args.length == 0){ // Just show the message.
				// TODO: Show message.
				if((!(guild_motd.containsKey(getGuild(p.getName())))) || guild_motd.get(getGuild(p.getName())) == null){
					// NO MOTD.
				...
				String g_name = getGuild(p.getName());
				String motd = guild_motd.get(g_name);
				p.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + guild_handle_map.get(g_name) + ChatColor.DARK_AQUA + "> " + ChatColor.BOLD + "MOTD: " + ChatColor.DARK_AQUA + motd);
				return true;
			}
			...
			if(args.length >= 1){
				...
				setGuildMOTD(getGuild(p.getName()), new_motd);
			}
		}
		...
		if(cmd.getName().equalsIgnoreCase("g")){
			...
			if(args.length == 0){
				if(!(guild_only.contains(p.getName()))){
					guild_only.add(p.getName());
					p.sendMessage(ChatColor.DARK_AQUA + "Messages will now be default sent to <" + ChatColor.BOLD + guild_handle_map.get(getGuild(p.getName())) + ChatColor.DARK_AQUA + ">. Type " + ChatColor.UNDERLINE + "/l <msg>" + ChatColor.DARK_AQUA + " to speak in local.");
					p.sendMessage(ChatColor.GRAY + "To change back to default local, type " + ChatColor.BOLD + "/g" + ChatColor.GRAY + " again.");
				}
				else if(guild_only.contains(p.getName())){
					guild_only.remove(p.getName());
		...
		if(cmd.getName().equalsIgnoreCase("guild")){
			...
			p.getInventory().addItem(guild_dye);
			p.getInventory().addItem(guild_emblem);
