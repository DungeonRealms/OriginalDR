		...
		try {
			...
			while ((inputLine = in.readLine()) != null) {
				...
				if (inputLine.equals("@rollout@")){
					...
					// Send it right away to prevent users from being directed to a rebooting server.
				...
				if(inputLine.startsWith("@mute@")){ // @mute@player_mutie/player_muted:unmute_time
					...
					if(Bukkit.getPlayer(p_name_muted) != null){
						...
						//int minutes_to_mute = ChatMechanics.minutesUntilUnmute(muted); DEPRECIATED
						muted.sendMessage("");
				...
				if(inputLine.startsWith("[professionbuff]")){
					...
					CommunityMechanics.instance.getServer().broadcastMessage("");
					CommunityMechanics.instance.getServer().broadcastMessage(ChatColor.GOLD + "" + ChatColor.BOLD + ">> " + "(" + from_server + ") " +  ChatColor.RESET + player_string + ChatColor.GOLD + " has just activated " + ChatColor.UNDERLINE + "+20% Global Mining / Fishing EXP Rates" + ChatColor.GOLD + " for 30 minutes by using 'Global Profession Buff' from the E-CASH store!");
					CommunityMechanics.instance.getServer().broadcastMessage("");
				...
				if(inputLine.startsWith("[lootbuff]")){
					...
					CommunityMechanics.instance.getServer().broadcastMessage("");
					CommunityMechanics.instance.getServer().broadcastMessage(ChatColor.GOLD + "" + ChatColor.BOLD + ">> " + "(" + from_server + ") " +  ChatColor.RESET + player_string + ChatColor.GOLD + " has just activated " + ChatColor.UNDERLINE + "+20% Global Drop Rates" + ChatColor.GOLD + " for 30 minutes by using 'Global Loot Buff' from the E-CASH store!");
					CommunityMechanics.instance.getServer().broadcastMessage("");
				...
				if(inputLine.startsWith("[addpet]")){
					...
					if(Hive.player_inventory.containsKey(p_name)){ // They're online!
						...
						Player pl = CommunityMechanics.instance.getServer().getPlayer(p_name);
				...
				if(inputLine.startsWith("[sq_online]")){
					...
					OfflinePlayer op = CommunityMechanics.instance.getServer().getOfflinePlayer(p_name);
					
					if(op.isOp()){
						return;
					}
					
					for(Player pl : CommunityMechanics.instance.getServer().getOnlinePlayers()){
				...
				if(inputLine.startsWith("[gdemote]")){
					...
					GuildMechanics.setGuildRank(p_name, rank); // rank == 2 for officer.
				...
				if(inputLine.startsWith("[gquit]")){
					...
					// Leave guild on every server so there's no data inconsistancies. FTW.
				...
				if(inputLine.startsWith("[quit]")){
					...
					for(String g_member : GuildMechanics.getGuildMembers(g_name)){
						...
						if(p_name.equalsIgnoreCase(pl_g_member.getName())){
							continue; // Don't tell ourselves. (server transfers)
						}

						/*if(CommunityMechanics.socialQuery(g_member, p_name, "CHECK_BUD")){
							continue; // They're buddies, let the buddy plugin take care of telling them they logged in.
						}*/
				...
				// Players should ONLY get messages about a player logging in/out if ONE player has them added and they are NOT on other player's ignore list.
				...
				if(inputLine.startsWith("#")){ // Reply to a "^" message.
					//#player_name_to_tell/player_name_about=offline OR #player_name_to_tell/player_name_about=online,US-0:MESSAGE
				...
				if(inputLine.startsWith("^")){ // Sending message.
					...
					if(!(PermissionMechanics.rank_map.containsKey(p_from_name))){
						PermissionMechanics.downloadRank(p_from_name); // Get rank data of player on local server.
					}
