    	...
    	// pending_upload(string)
		  ...
		  List<String> to_fix = new ArrayList<String>();
	        ...
	        for(Player pl : Bukkit.getOnlinePlayers()){
	        	if(pl.getPlayerListName().equalsIgnoreCase("")){continue;} // NPC.
	        	if(pl.hasMetadata("NPC")){continue;}
	        	actual_online.add(pl.getName());
	        }
	        
	        for(String s : reported_online){
	        	if(Hive.pending_upload.contains(s) && sql_reported_online.contains(s)){continue;} // They are pending upload and still marked as 'online' this server, so the upload has not finished.
