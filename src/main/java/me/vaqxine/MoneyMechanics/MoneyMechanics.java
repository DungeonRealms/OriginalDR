...
public class MoneyMechanics implements Listener {
    ...
    public static ItemStack t1_gem_pouch = ItemMechanics.signCustomItem(Material.INK_SACK, (short) 0, ChatColor.WHITE.toString() + "Small Gem Pouch"
            + ChatColor.GREEN + ChatColor.BOLD.toString() + " 0g", ChatColor.GRAY.toString() + "A small linen pouch that holds " + ChatColor.BOLD + "100g");

    public static ItemStack t2_gem_pouch = ItemMechanics.signCustomItem(Material.INK_SACK, (short) 0, ChatColor.GREEN.toString() + "Medium Gem Sack"
            + ChatColor.GREEN + ChatColor.BOLD.toString() + " 0g", ChatColor.GRAY.toString() + "A medium wool sack that holds " + ChatColor.BOLD + "150g");

    public static ItemStack t3_gem_pouch = ItemMechanics.signCustomItem(Material.INK_SACK, (short) 0, ChatColor.AQUA.toString() + "Large Gem Satchel"
            + ChatColor.GREEN + ChatColor.BOLD.toString() + " 0g", ChatColor.GRAY.toString() + "A large leather satchel that holds " + ChatColor.BOLD + "200g");

    public static ItemStack t4_gem_pouch = ItemMechanics.signCustomItem(Material.INK_SACK, (short) 0, ChatColor.LIGHT_PURPLE.toString()
            + "Gigantic Gem Container" + ChatColor.GREEN + ChatColor.BOLD.toString() + " 0g", ChatColor.GRAY.toString() + "A giant container that holds "
            + ChatColor.BOLD + "300g");
    ...
    public static ItemStack generateArrowButton(String type, int current_page, int max_pages) {
        if (type.equalsIgnoreCase("next")) {
            return ItemMechanics.signCustomItem(Material.ARROW, (short) 1, ChatColor.YELLOW.toString() + "Next Page " + ChatColor.BOLD.toString() + "->",
                    ChatColor.GRAY.toString() + "Page " + current_page + "/" + max_pages);
    ...
    public static ItemStack generateStaticCashStack(int new_amount) {
        return ItemMechanics.signCustomItem(
                Material.EMERALD,
                (short) 0,
                ChatColor.GREEN.toString() + new_amount + ChatColor.GREEN.toString() + "" + ChatColor.BOLD.toString() + " GEM(s)",
                ChatColor.GRAY.toString() + "" + ChatColor.GREEN.toString() + "Left Click" + ChatColor.GRAY.toString() + " to withdraw " + ChatColor.GREEN
                        + ChatColor.BOLD + "RAW GEMS" + "," + ChatColor.GRAY.toString() + "" + ChatColor.GREEN.toString() + "Right Click"
                        + ChatColor.GRAY.toString() + " to create " + ChatColor.GREEN + ChatColor.BOLD + "A GEM NOTE" + "," + ChatColor.GREEN.toString()
    ...
    public static boolean downloadBankDatabaseData(final String p_name) {
        ...
        try {
            ...
            if (level == -2) {
                ...
                level = 0;
                while (bank_content != null && bank_content.contains("@item@") && (bank_content.split("@item@").length > (getBankSlots(level) - 1))
                        && level < 17) {
                    level++;
                }
            }
                ...
                if (bank_content.contains("@page_break@")) {
                    ...
                    for (String s_bank_content : bank_content.split("@page_break@")) {
                        if (s_bank_content.contains("@item@")) {
                            ...
                            Inventory page = Hive.convertStringToInventory(null, s_bank_content, "Bank Chest (" + current_page + "/" + total_pages + ")",
                                    page_slots);
    ...
    public static void addMoneyToOfflinePlayerBank(final String p_name, int amount_to_add) {
        Hive.sql_query.add("INSERT INTO bank_database (p_name, money)" + " VALUES" + "('" + p_name + "', '" + amount_to_add
                + "') ON DUPLICATE KEY UPDATE money=bank_database.money+" + amount_to_add + "");
    ...
    public static void uploadBankDatabaseData(final String p_name, boolean remove_when_done) {
        ...
        try {
            pst = ConnectionPool.getConnection().prepareStatement(
                    "INSERT INTO bank_database (p_name, content, money, level)" + " VALUES" + "('" + p_name + "', '"
                            + StringEscapeUtils.escapeSql(final_bank_content) + "', '" + final_bank_net + "', '" + final_bank_level
                            + "') ON DUPLICATE KEY UPDATE content = '" + StringEscapeUtils.escapeSql(final_bank_content) + "', money='" + final_bank_net
    ...
    public ItemStack setCountMeta(ItemStack is, int amount) {
        net.minecraft.server.v1_7_R2.ItemStack nmsstack = CraftItemStack.asNMSCopy(is);
    ...
    public static boolean isGemPouch(ItemStack is) {
        if (is.getType() == Material.INK_SACK && is.getDurability() == (short) 0 && is.hasItemMeta() && is.getItemMeta().hasDisplayName()
                && is.getItemMeta().getDisplayName().endsWith("g")) {
            return true;
        }
        return false;
    }
    ...
    public static void addMoneyCert(Player p, int amount, boolean echo) {
        ...
        if (p.getInventory().firstEmpty() == -1) {
            p.getWorld().dropItem(
                    p.getLocation(),
                    signBankNote(money, ChatColor.GREEN.toString() + "Bank Note", ChatColor.WHITE.toString() + ChatColor.BOLD.toString() + "Value:"
                            + ChatColor.WHITE.toString() + " " + amount + " Gems" + "," + ChatColor.GRAY.toString() + "Exchange at any bank for GEM(s)"));
    ...
    public static void updateMoney(Player p, int slot, int new_amount) {
        p.getInventory().setItem(
                slot,
                signBankNote(new ItemStack(Material.PAPER, 1, (short) 777), ChatColor.GREEN.toString() + "Bank Note", ChatColor.WHITE.toString()
                        + ChatColor.BOLD.toString() + "Value:" + ChatColor.WHITE.toString() + " " + new_amount + " Gems" + "," + ChatColor.GRAY.toString()
                        + "Exchange at any bank for GEM(s)"));
        // addMoneyCert(p, new_amount, false); Depreciated.
    }

    public static boolean isNextArrow(ItemStack is) {
        if (is != null && is.getType() == Material.ARROW && is.hasItemMeta() && is.getItemMeta().hasDisplayName() && is.getDurability() == (short) 1
                && is.getItemMeta().getDisplayName().equalsIgnoreCase(ChatColor.YELLOW.toString() + "Next Page " + ChatColor.BOLD.toString() + "->")) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isPreviousArrow(ItemStack is) {
        if (is != null
                ...
                && is.getDurability() == (short) 1
                && is.getItemMeta().getDisplayName()
                        .equalsIgnoreCase(ChatColor.YELLOW.toString() + ChatColor.BOLD + "<-" + ChatColor.YELLOW.toString() + " Previous Page ")) {
            return true;
        }
        return false;
    }
        ...
        if (e.getItemDrop().getItemStack().getType() == Material.PAPER) {
            ...
            if (split_map.containsKey(p)) {
                p.sendMessage(ChatColor.RED
                        + "Please finish splitting your bank note before attempting to drop any other notes. Type 'cancel' to void the split.");
                e.setCancelled(true);
            }
        }
    }
    ...
    public void upgradeBank(Player p, int new_level, boolean interval) {
        ...
        if (new_level == 6 || new_level == 12) {
            ...
            for (ItemStack is : bank_contents.get(p.getName()).get(last_page_index)) {
                if (is != null && !(is.getType() == Material.AIR) && is.getType() != Material.EMERALD && !isDivider(is)
                        && !isPreviousArrow(is) && !isNextArrow(is)) {
                    i.setItem(i.firstEmpty(), is);
                }
            }
    ...
    public static boolean isDivider(ItemStack is) {
        if (is.hasItemMeta() && is.getItemMeta().hasDisplayName() && is.getItemMeta().getDisplayName().equalsIgnoreCase(" ")
                && is.getType() == Material.THIN_GLASS) {
            return true;
        }
        return false;
    }
        ...
        while (Gems_worth > 0) {
            while (Gems_worth > 64) {
                if (p.getInventory().firstEmpty() == -1) {
                    ...
                    p.sendMessage(ChatColor.RED + "" + ChatColor.UNDERLINE + "WARNING: " + ChatColor.RED
                            + "Not all GEMS fit in your inventory, the remainder have been dropped on the ground nearby.");
                    Gems_worth = 0;
                }
    ...
    public void onPlayerChatEvent(AsyncPlayerChatEvent e) {
        Player p = e.getPlayer();
        if (!split_map.containsKey(p) && !withdraw_map.containsKey(p) && !bank_upgrade_codes.containsKey(p)) {
            return;
        }
        e.setCancelled(true);
        ...
        if (split_map.containsKey(p)) {
            ...
            p.getInventory().setItem(
                    slot,
                    signBankNote(i, ChatColor.GREEN.toString() + "Bank Note", ChatColor.WHITE.toString() + ChatColor.BOLD.toString() + "Value:"
                            + ChatColor.WHITE.toString() + " " + dif + " Gems" + "," + ChatColor.GRAY.toString() + "Exchange at any bank for GEM(s)"));
        }
    ...
    public void onPlayerInteract(PlayerInteractEvent e) {
        ...
        if (e.hasItem() && isGemPouch(e.getItem())) {
            ...
            pl.sendMessage(ChatColor.YELLOW + "To " + ChatColor.BOLD + "DEPOSIT" + ChatColor.YELLOW
                    + " GEM(s), drag them onto the pouch in your inventory. To " + ChatColor.BOLD + "WITHDRAW" + ChatColor.YELLOW + " GEM(s), "
                    + ChatColor.UNDERLINE + "right click" + ChatColor.YELLOW + " the pouch in your inventory.");
            pl.sendMessage(ChatColor.GRAY
                    + "You can deposit gems in pouches directly into your bank by dragging them into your bank chest like you would GEM NOTES.");
            return;
        }
        ...
        if (i.getType() == Material.PAPER && (e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK)) {
            ...
            if (withdraw_map.containsKey(p)) {
                p.sendMessage(ChatColor.RED
                        + "You cannot split bank notes while you have a pending withdrawl request. Type 'cancel' or finish that operation first.");
                return;
            }
            ...
            p.sendMessage(ChatColor.GRAY + "This bank note is worth " + ChatColor.GREEN + cert_val + " Gems." + ChatColor.GRAY
                    + " Please enter the amount you'd like to sign an additional bank note for. Alternatively, type " + ChatColor.RED + "'cancel'"
                    + ChatColor.GRAY + " to stop this operation.");
            split_map.put(p, 0);
    ...
    public void onPlayerOpenChest(PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.LEFT_CLICK_BLOCK && e.getClickedBlock().getType() == Material.ENDER_CHEST) {
            ...
            if (!ShopMechanics.hasCollectionBinItems(p.getName())) {
                p.sendMessage(ChatColor.RED + "You have no items in your collection bin. To access your bank account, " + ChatColor.UNDERLINE + "RIGHT CLICK"
                        + ChatColor.RED + " the bank chest.");
                return;
            }
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock().getType() == Material.ENDER_CHEST) {
            ...
            if (p.isSneaking() && !TutorialMechanics.onTutorialIsland(p)) {
                ...
                p.sendMessage(ChatColor.DARK_GRAY + "           *** " + ChatColor.GREEN + ChatColor.BOLD + "Bank Upgrade Confirmation" + ChatColor.DARK_GRAY
                        + " ***");
                p.sendMessage(ChatColor.DARK_GRAY + "           CURRENT Slots: " + ChatColor.GREEN + getBankSlots(bank_tier) + ChatColor.DARK_GRAY
                        + "          NEW Slots: " + ChatColor.GREEN + getBankSlots(next_bank_tier));
                ...
                p.sendMessage("");
                p.sendMessage(ChatColor.GREEN + "Enter the code '" + ChatColor.BOLD + getUpgradeAuthenticationCode(p) + ChatColor.GREEN
                        + "' to confirm your upgrade.");
                p.sendMessage("");
                p.sendMessage("" + ChatColor.RED + ChatColor.BOLD + "WARNING:" + ChatColor.RED + " Bank upgrades are " + ChatColor.BOLD + ChatColor.RED + "NOT"
    ...
    public boolean isStaticGem(ItemStack is) {
        if (is != null && is.getType() == Material.EMERALD && is.hasItemMeta() && is.getItemMeta().hasDisplayName()
                && is.getItemMeta().getDisplayName().endsWith(ChatColor.BOLD.toString() + " GEM(s)")) {
            return true;
        }
        return false;
    }
    ...
    public void onInventoryCloseEvent(InventoryCloseEvent e) throws IOException {
        ...
        if (e.getInventory().getName().startsWith("Bank Chest")) {
            if (e.getInventory().firstEmpty() == -1 && e.getInventory().getSize() <= 45) {
                ...
                p.sendMessage(ChatColor.GRAY + "Banker: " + ChatColor.WHITE + "To purchase more bank slots, " + ChatColor.GREEN + ChatColor.BOLD
                        + "SNEAK + RIGHT-CLICK" + ChatColor.WHITE + " the banking chest.");
            }
            ...
            if (e.getInventory().contains(Material.PAPER)) {
                for (Entry<Integer, ? extends ItemStack> data : e.getInventory().all(Material.PAPER).entrySet()) {
                    ...
                    if (is.hasItemMeta() && is.getItemMeta().hasDisplayName() && is.getItemMeta().getDisplayName().contains("Bank Note")) {
                        ...
                        net += amount;
                        p.sendMessage(ChatColor.GREEN + "" + ChatColor.BOLD + "+" + ChatColor.GREEN + amount + ChatColor.BOLD + "G" + ChatColor.GREEN + ", "
                                + ChatColor.BOLD + "New Balance: " + ChatColor.GREEN + net + " GEM(s)");
        ...
        if ((e.isShiftClick() && e.getCurrentItem() != null) || (e.getCursor() != null && e.getCurrentItem() != null)) {
            ...
            if (e.isShiftClick()) {
                ...
                if (e.getInventory().getName().contains("@") || e.getInventory().getName().contains("Collection Bin")
                        || e.getInventory().getName().contains(pl.getName()) || e.getInventory().getName().contains("Merchant")) {
    ...
    public void onGemPouchWithdraw(InventoryClickEvent e) {
        Player pl = (Player) e.getWhoClicked();
        if (e.isLeftClick() || e.isShiftClick() || (e.getCursor() != null && e.getCursor().getType() != Material.AIR) || e.getCurrentItem() == null
    ...
    public void onGemPouchDeposit(InventoryClickEvent e) {
        Player pl = (Player) e.getWhoClicked();
        if (e.isRightClick() || e.getCursor() == null || e.getCurrentItem() == null || e.getCursor().getType() != Material.EMERALD
                || !(isGemPouch(e.getCurrentItem()))) {
                ...
                if (e.getClick() == ClickType.MIDDLE) { // TODO: Is this middle click?
                    ...
                    Main.plugin.getServer().getScheduler().scheduleSyncDelayedTask(Main.plugin, new Runnable() {
                        public void run() {
                            ...
                            p.playSound(p.getLocation(), Sound.CHEST_CLOSE, 1F, 1F);
                            p.sendMessage("");
                            p.sendMessage(ChatColor.DARK_GRAY + "           *** " + ChatColor.GREEN + ChatColor.BOLD + "Bank Upgrade Confirmation"
                                    + ChatColor.DARK_GRAY + " ***");
                            p.sendMessage(ChatColor.DARK_GRAY + "           CURRENT Slots: " + ChatColor.GREEN + getBankSlots(bank_tier) + ChatColor.DARK_GRAY
                                    + "          NEW Slots: " + ChatColor.GREEN + getBankSlots(next_bank_tier));
                            // p.sendMessage(ChatColor.DARK_GRAY + "FROM Tier " + ChatColor.GREEN + bank_tier + ChatColor.DARK_GRAY + " TO " + ChatColor.GREEN +
                            ...
                            p.sendMessage(ChatColor.GREEN + "Enter the code '" + ChatColor.BOLD + getUpgradeAuthenticationCode(p) + ChatColor.GREEN
                                    + "' to confirm your upgrade.");
                            p.sendMessage("");
                            p.sendMessage("" + ChatColor.RED + ChatColor.BOLD + "WARNING:" + ChatColor.RED + " Bank upgrades are " + ChatColor.BOLD
                                    + ChatColor.RED + "NOT" + ChatColor.RED + " reversible or refundable. Type 'cancel' to void this upgrade request.");
                            p.sendMessage("");
                        }
                    }, 2L);
                    ...
                    Main.plugin.getServer().getScheduler().scheduleAsyncDelayedTask(Main.plugin, new Runnable() {
                        public void run() {
                            ...
                            p.sendMessage(ChatColor.GRAY + "Banker: " + ChatColor.WHITE + "How much would you like to WITHDRAW today, " + p.getDisplayName()
                                    + "?");
                            p.sendMessage(ChatColor.GRAY + "Please enter the amount you'd like To WITHDRAW. Alternatively, type " + ChatColor.RED + "'cancel'"
                                    + ChatColor.GRAY + " to void this operation.");
                        }
                    }, 2L);
                    ...
                    Main.plugin.getServer().getScheduler().scheduleAsyncDelayedTask(Main.plugin, new Runnable() {
                        ...
                        public void run() {
                            ...
                            p.sendMessage(ChatColor.GRAY + "Banker: " + ChatColor.WHITE + "How much would you like to CONVERT today, " + p.getDisplayName()
                                    + "?");
                            p.sendMessage(ChatColor.GRAY + "Please enter the amount you'd like To CONVERT into a gem note. Alternatively, type "
    ...
    public void onPlayerClickInventory(InventoryClickEvent event) {
        ...
        if (event.getCursor() != null && event.getCurrentItem() != null && event.getCursor().getType() == Material.PAPER
                && event.getCurrentItem().getType() == Material.PAPER && !TradeMechanics.trade_map.containsKey(p)) {
            if (split_map.containsKey(p)) {
            ...
            event.setCurrentItem(signBankNote(event.getCurrentItem(), ChatColor.GREEN.toString() + "Bank Note",
                    ChatColor.WHITE.toString() + ChatColor.BOLD.toString() + "Value:" + ChatColor.WHITE.toString() + " " + new_val + " Gems" + ","
                            + ChatColor.GRAY.toString() + "Exchange at any bank for GEM(s)"));
            // p.getInventory().getItem(event.getSlot()).setAmount(1);
            p.updateInventory();
            p.sendMessage(ChatColor.GRAY + "You've combined bank notes " + ChatColor.ITALIC + map1_val + "G + " + map2_val + "G" + ChatColor.GRAY
                    + " into one bank note with the value of " + ChatColor.BOLD + new_val + "G.");
            p.playSound(p.getLocation(), Sound.ENDERDRAGON_WINGS, 1F, 1.2F);
            return;
        }
        ...
        if (event.getInventory().getName().startsWith("Bank Chest")
                && (((event.getCursor() != null && event.getCursor().getType() == Material.EMERALD)
                        || (event.getCursor() != null && event.getCursor().getType() == Material.PAPER) || (event.getCursor() != null && isGemPouch(event
                        .getCursor()))) || (event.isShiftClick() && ((event.getCurrentItem() != null && event.getCurrentItem().getType() == Material.EMERALD)
                        || (event.getCurrentItem() != null && event.getCurrentItem().getType() == Material.PAPER) || (event.getCurrentItem() != null && isGemPouch(event
                        .getCurrentItem())))))) {
            ...
            if (event.getSlotType() == SlotType.OUTSIDE || event.getSlotType() == SlotType.ARMOR || event.getSlotType() == SlotType.FUEL
            ...
            if (amount_to_deposit > 0) {
                ...
                p.sendMessage(ChatColor.GREEN + "" + ChatColor.BOLD + "+" + ChatColor.GREEN + amount_to_deposit + ChatColor.BOLD + "G" + ChatColor.GREEN + ", "
                        + ChatColor.BOLD + "New Balance: " + ChatColor.GREEN + (amount_to_deposit + cur_val) + " GEM(s)");
                ...
                public void run() {
                    if ((it == null || it.isDead())) {
                        ...
                        if (CommunityMechanics.toggle_list.containsKey(p.getName()) && CommunityMechanics.toggle_list.get(p.getName()).contains("debug")) {
                            p.sendMessage(ChatColor.GREEN + "" + ChatColor.BOLD + "                                 +" + ChatColor.GREEN + (int) (amount)
                                    + ChatColor.BOLD + "G" + ChatColor.GREEN);
                        }
                    }
                }
            }, 2L);
