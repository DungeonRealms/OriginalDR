	...
	public void run() {
		if(ShopMechanics.need_sql_update.size() > 0) {
			...
			for(String shop_owner : ShopMechanics.need_sql_update) {
				...
				try {
					pst = ConnectionPool.getConnection().prepareStatement("INSERT INTO shop_database (p_name, shop_backup, collection_bin)" + " VALUES" + "('" + shop_owner + "', '" + StringEscapeUtils.escapeSql(shop_contents) + "', '" + StringEscapeUtils.escapeSql(collection_bin_s) + "') ON DUPLICATE KEY UPDATE shop_backup = '" + StringEscapeUtils.escapeSql(shop_contents) + "', collection_bin='" + StringEscapeUtils.escapeSql(collection_bin_s) + "'");
