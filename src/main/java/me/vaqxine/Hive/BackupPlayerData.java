...
public class BackupPlayerData extends Thread {
	...
	public static void backupPlayers(Player[] data){
		for(Player p : data){
			...
			try{Hive.uploadPlayerDatabaseData(p_name);} catch(SQLException err){err.printStackTrace();}; // Location, Inventory
	...
	public void run(){
		while(true){
			try {Thread.sleep(900 * 1000);} catch (InterruptedException e) {}
