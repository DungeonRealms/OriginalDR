...
public class ItemGenerators {
	...
	public static ItemStack customGenerator(String template_name) {
		...
		if(!(template.exists())) { return null; // No such custom template!
		}
	...
	public static ItemStack MobChestPlateGenerator(int tier, boolean reroll, ItemStack original, int mob_tier_level) {
        Random r = new Random();
        Material m = null;
	...
	public static ItemStack MobHelmetGenerator(int tier, boolean reroll, ItemStack original, int mob_level_tier) {
        ...
        if(armor_range_check == 99 && mob_level_tier >= 4) {
            rarity = ChatColor.YELLOW.toString() + ChatColor.ITALIC.toString() + "Unique";
            if(tier == 1) {
                int min_num = 1 + 1;
                int max_num = 2 + 1;
