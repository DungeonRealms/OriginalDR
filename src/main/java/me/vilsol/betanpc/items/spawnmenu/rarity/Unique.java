...
public class Unique implements MenuItem {
	...
	public void execute(Player plr, ClickType click) {
		...
		if(ItemType.isArmor(m.type.getDRType())){
			plr.getInventory().addItem(m.type.getDRType().generateArmor(m.tier.getDRTier(), null, -1, ItemRarity.UNIQUE));
		}else{
			plr.getInventory().addItem(m.type.getDRType().generateWeapon(m.tier.getDRTier(), null, ItemRarity.UNIQUE));
		}
