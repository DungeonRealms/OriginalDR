...
public class FatigueMechanics extends JavaPlugin implements Listener {
	...
	public static void removeEnergy(Player p, float remove){
		...
		p.setMetadata("last_energy", new FixedMetadataValue(instance, System.currentTimeMillis()));
	...
	public void replenishEnergy(){
		for(Player p : this.getServer().getOnlinePlayers()){
			if(p.getPlayerListName().equalsIgnoreCase("")){continue;}
	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent e){
		if(e.getDamager() instanceof Player){
			...
			if(!e.isCancelled() && e.getDamage() > 0 && p.getItemInHand() != null && p.getItemInHand().getType() != Material.AIR){
				...
				if(in_hand.getType() == Material.WOOD_SWORD || in_hand.getType() == Material.STONE_SWORD || in_hand.getType() == Material.IRON_SWORD || in_hand.getType() == Material.DIAMOND_SWORD || in_hand.getType() == Material.GOLD_SWORD 
						|| in_hand.getType() == Material.WOOD_AXE || in_hand.getType() == Material.STONE_AXE || in_hand.getType() == Material.IRON_AXE || in_hand.getType() == Material.DIAMOND_AXE || in_hand.getType() == Material.GOLD_AXE
						|| in_hand.getType() == Material.WOOD_SPADE || in_hand.getType() == Material.STONE_SPADE || in_hand.getType() == Material.IRON_SPADE || in_hand.getType() == Material.DIAMOND_SPADE || in_hand.getType() == Material.GOLD_SPADE
						|| in_hand.getType() == Material.BOW){
					// It's a weapon!
